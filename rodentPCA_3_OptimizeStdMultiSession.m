...
Arrange1 = Data1.Arrange;

load(fullfile(path2,file2)); 
Data2 = Data;
numNeuron2 = size(Data2.binned,1);
Arrange2 = Data2.Arrange;

if ~strcmp(Arrange1, Arrange2) % the '~' symbol means 'not'
    error('Stack error') % This line will display 'Stack error' and stop execution
end
...
sigma_vec1 = linspace(1,2,2);%50 points from 1ms to 50ms
sigma_vec2 = logspace(log10(20),log10(200),1);% 30 points from 50ms to 400ms with log scale
...
for std = 1:length(sigma_vec)
    ...
    for i = 1:length(ind) % in most cases, n=8, which corresponding to 8 events
...
[coeff1R,score1R,explained1R,m1R] = PCA(EventOneMatrix',1,0.6);
[coeff2R,score2R,explained2R,m2R] = PCA(EventTwoMatrix',1,0.6);
[coeff1N,score1N,explained1N,m1N] = PCA(NoiseOneMatrix',1,0.6);
[coeff2N,score2N,explained2N,m2N] = PCA(NoiseTwoMatrix',1,0.6);
mR = min(m1R,m2R);
mN = min(m1N,m2N);
numPC = min(mR,mN);  % average the top 4 PC for comparison
numPC = min(numPC,4); 
...
[coeff1R,score1R,explained1R,m1R] = PCA(EventOneMatrix',1,0.7);
[coeff2R,score2R,explained2R,m2R] = PCA(EventTwoMatrix',1,0.7);
[coeff1N,score1N,explained1N,m1N] = PCA(NoiseOneMatrix',1,0.7);
[coeff2N,score2N,explained2N,m2N] = PCA(NoiseTwoMatrix',1,0.7);
mR = min(m1R,m2R);
mN = min(m1N,m2N);
numPC = min(mR,mN);  % average the top 4 PC for comparison
numPC = min(4,numPC);
...
%% Calculate PCA using different mode
% for i = 1:length(ind)
...
% CCA
[rA,rB,rU,rV] = CCA(EventOneLD{i},EventTwoLD{i},mR);
[nA,nB,nU,nV] = CCA(NoiseOneLD{i},NoiseTwoLD{i},mN);
...
% calculate correlation coefficient for noise Latent Dynamics
%R_LDnoise = corrcoef(NoiseOneLDbin{i},NoiseTwoLDbin{i});
%r_LDnoise{std}(i) = R_LDnoise(1,2);
%r_LDnoise(i) = R_LDnoise(1,2);

% calculate correlation coefficient for noise Canonoical Variables
...
% Calculate combined correlation coefficient for Latent Dynamics
%R_LDreal = corrcoef(EventOneLDcom,EventTwoLDcom);
%r_LDreal = [r_LDreal,R_LDreal(1,2)];

% Calculate combined correlation coefficient for Canonical Variables
%R_CVreal = corrcoef(EventOneCVcom,EventTwoCVcom);
%r_CVreal = [r_CVreal{std},R_CVreal(1,2)];

% Calculate combined correlation coefficient for noise Latent Dynamics
%R_LDnoise = corrcoef(NoiseOneLDcom,NoiseTwoLDcom);
%r_LDnoise = [r_LDnoise,R_LDnoise(1,2)];

% Calculate combined correlation coefficient for noise Canonical Variables
...
%% Calculate coefficient for weighted CV 
...
switch Arrange
    case 'neuron'
         fig2_name = sprintf('%s vs. %s correlation difference(log)stackneuron-bin%dms15 trials',...
    newStr1,newStr2,bin_size_ms);
         mat_name = sprintf('%s vs %s_%sSmoothOptStd_stackneuron',newStr1,newStr2,binStr);

    case 'time'
        fig2_name = sprintf('%s vs. %s correlation difference(log)stacktime-bin%dms15 trials',...
    newStr1,newStr2,bin_size_ms);
        mat_name = sprintf('%s vs %s_%sSmoothOptStd_stacktime',newStr1,newStr2,binStr);
